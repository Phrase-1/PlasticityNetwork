...
#include<time.h>
#include<windows.h>
clock_t start, stop;//用于计时
double duration;
int stay_still_round = 0;//记录躺平次数，过多时适应度清零
#define MAP_SIZE 100
#define FOOD_NUMBER 50
#define FOOD_REFRESH_RATE 50//食物刷新频率，每50步刷新
#define ORIENT_UP 0
...
#define DELIBRATION_STEP 5//最小为1，为0时不学习
...
void InitFoods(Food *foods, Organism o){
    ...
    while(1){
        ...
        for(int i = 0; i < generated_times; i++){
            ...
            if(((float)RandNum_x==foods[i].coordinate.x)&&((float)RandNum_y==foods[i].coordinate.y)){
                flag = 1;
            }
...
void OrganMove(Organism *o, int command_code1, int command_code2){
    ...
    if(command_code1==1 && command_code2==1){
        ...
        if(!BorderDetection(temp_x, temp_y)){
            ...
            //经实验验证不可行，生物只会前进，吃掉前进路径上的的食物
            //o->orient.orient_status = (o->orient.orient_status+180) % 360;
            ;
        }

    }
    else{stay_still_round++;}
}
//提示最近食物位置_笛卡尔坐标系
//轮流比较曼哈顿距离
...
void PrintMap(Food foods[], Organism o, float sensory_array[], int action_steps, int generation, int individual)
{
    ...
    printf("the organism is aheading %d\n", o.orient.orient_status);
...
void PrintFullMap(Food foods[], Organism o, float sensory_array[], int action_steps, int generation, int individual)
{
    printf("this is %dth individual of %dth generation\n", individual, generation);
    int pointPrinted = 0;
    //x为行y为列//打印整个地图
...
//保存每代平均适应度
void SaveAverageResult(Genome *Population, int current_generation, int forage_successful_individual){
    ...
    fprintf(fp, "%d\t", forage_successful_individual);
    char ch = '\n';
    fputc(ch, fp);
    fclose(fp);
}
...
void SaveBestOfEachGeneration(Genome *Population, int current_generation){
    int index = FindBestIndividual(Population);
    FILE *fp;
    fp = fopen("BestResultOfEachGeneration.txt","a");
    fprintf(fp, "%d\t",current_generation);//generation
    fprintf(fp, "%f\t", Population[index].fitness);//fitness
    for(int i = 0; i < GENE_LENGTH; i++){
        fprintf(fp, "%lf\t", Population[index].gene_info[i]);//gene
    }
    char ch = '\n';
    fputc(ch, fp);
...
void StartNewExperiment(){
    ...
    int forage_successful_individual;//记录成功吃到的生物个数
    while(current_generation < MAX_GENERATION){
        start=clock();
        current_individual = 0;
        forage_successful_individual = 0;
        //从第二代开始不初始化，而进行选择交叉变异
        while(current_individual < GENE_NUMBER){
            //对每个生物体初始化状态，大脑，可塑性网络
            stay_still_round = 0;//每个个体重置躺平次数
            action_steps = 0;
            
            InitOrgan(&o);
            
            InitFoods(foods,o);
            InitiateRecurrentNeuralNetwork(&rnn,Population[current_individual]);
            ...
            while(action_steps < ORGAN_LIFE_SPAN){
                // if(stay_still_round % 100 == 0){o.surplus -= 0.005;}
                BrainReceiveInput(&rnn, 0, 0, sensory_array);
                for(int i = 0; i < DELIBRATION_STEP; i++){//Delibration Steps
                    BrainFeedForward(&rnn);//大脑网络进行运算
                    previous_action[0] = rnn.Neurons[7];
                    previous_action[1] = rnn.Neurons[8];
                    UpdateRecurrentBrainNetwork(&rnn, &pn);//使用可塑性网络更改大脑的w与b
                    UpdateOrganSurplus(&o, LEARNING_COST);//计算能量消耗
                    //更新权重之后再用大脑计算行动结果
                    BrainReceiveInput(&rnn, previous_action[0], previous_action[1], sensory_array);
                } 
                BrainFeedForward(&rnn);
                OrganMove(&o,rnn.Neurons[7],rnn.Neurons[8]);
                previous_action[0] = rnn.Neurons[7];
                previous_action[1] = rnn.Neurons[8];
                ...
                if (action_steps % FOOD_REFRESH_RATE == 0)
                {
                    InitFoods(foods, o);
                }
                //Sleep(3000);
            ...
            if(o.eaten_number > 0){
                Population[current_individual].once_ate = 1;
                forage_successful_individual++;
            }
        ...
        SaveAverageResult(Population, current_generation, forage_successful_individual);

        // for(int i = 0; i < 60; i++){
        //     printf("%f ", Population[0].gene_info[i]);
        // }
        // printf("\n");
        // for(int i = 0; i < 60; i++){
        //     printf("%f ", Population[1].gene_info[i]);
        // }
        // printf("\n");
        // printf("\n");
        // printf("\n");
        BinaryCrossPopulation(Population);
        // for(int i = 0; i < 60; i++){
        //     printf("%f ", Population[0].gene_info[i]);
        // }
        // printf("\n");
        ...
        // }
        // printf("\n");
        BinaryMutatePopulation(Population);
        printf("generation %d finished\t", current_generation);
        stop=clock();
        duration=(double)(stop-start)/CLK_TCK; //CLK_TCK为clock()函数的时间单位，即时钟打点
        printf("time cost:%f\n",duration);
...
void TestIndividual()
{
    ...
    PlasticityNetwork pn;
    Genome g;
    FILE *fp = fopen("IndividualTest.txt", "r");
    for (int i = 0; i < GENE_LENGTH; i++)
    {
        fscanf(fp, "%f", &g.gene_info[i]);
        printf("%f\t", g.gene_info[i]);
    }
    printf("\n");
    fclose(fp);
    ...
    InitiateRecurrentNeuralNetwork(&rnn, g);
    InitializePlasticityNet(&pn, g);
    InitOrgan(&o);
    InitFoods(foods, o);
    int action_steps = 0;
    float sensory_array[2];          // s[0]为上下，s[1]为左右
    int previous_action[2] = {0, 0}; //存储前一步操作，初始化为00
    Cartetian_Sensory(sensory_array, foods, o);
    // PrintMap(foods, o, sensory_array, action_steps, -1, -1);
    while (action_steps < ORGAN_LIFE_SPAN)
    {
        BrainReceiveInput(&rnn, previous_action[0], previous_action[1], sensory_array);
        for (int i = 0; i < DELIBRATION_STEP; i++)
        {                           // Delibration Steps
            BrainFeedForward(&rnn); //大脑网络进行运算
            previous_action[0] = rnn.Neurons[7];
            previous_action[1] = rnn.Neurons[8];
        ...
        //应加入offset
        system("cls");
        if(action_steps < 6000){
            PrintMap(foods, o, sensory_array, action_steps, -1, -1);
            printf("output1=%lf\noutput2=%lf\n", rnn.Neurons[7], rnn.Neurons[8]);
            printf("fitness=%f\n", GetFitness(o));
        }
        else{
            PrintFullMap(foods, o, sensory_array, action_steps, -1, -1);
            printf("output1=%lf\noutput2=%lf\n", rnn.Neurons[7], rnn.Neurons[8]);
            printf("fitness=%f\n", GetFitness(o));
            getchar();
        }
...
void main(){
    ...
    printf("1. Start new experiment\n");
    printf("2. Test individual\n");
    int cho;
    scanf("%d", &cho);
    switch(cho){
        case 1:
