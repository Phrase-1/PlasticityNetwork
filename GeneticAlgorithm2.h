...
{
    float brain_input_weight[4][3];//1 2层之间
    float brain_input_bias[4][3];
    float brain_initial_conn_weight[3][2];//2 3层之间
    float brain_initial_conn_bias[3][2];
    float plasticity_L1L2_weight[5][4];
    float plasticity_L1L2_bias[5][4];
    float plasticity_L2L3_weight[4][2];
    float plasticity_L2L3_bias[4][2];
    int brain_conn_matrix_L2_L3[3][2];
    float gene_info[98];//以上信息按顺序写入此数组，作为基因
    float fitness;
}Genome;
...
void InitPopulation(Genome *Population){
    for(int i = 0; i < GENE_NUMBER; i++){
        ...
        Population[i].fitness = 0;//randint(1,100);
    }
}
