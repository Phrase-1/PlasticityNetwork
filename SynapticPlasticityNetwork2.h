...
typedef struct PlasticityNetwork{
    float w_L1_L2[5][4];//一二层之间的连接矩阵
    float b_L1_L2[5][4];//一二层间bias矩阵
    float w_L2_L3[4][2];//二三层间连接矩阵
    float b_L2_L3[4][2];//二三层间bias矩阵
    float neuron_L1[5];//第一层 四个input
    float neuron_L2[4];//3个隐藏层
...
void InitializePlasticityNet(PlasticityNetwork *pn, Genome g){
    ...
    for(int i = 0; i < 5; i++){
        for(int j = 0; j < 4; j++){
            pn->w_L1_L2[i][j] = g.gene_info[index];
            index++;
        }
    }
    index = 24;
    for(int i = 0; i < 5; i++){
        for(int j = 0; j < 4; j++){
            pn->b_L1_L2[i][j] = g.gene_info[index];
            index++;
        }
    }
    index = 44;
    for(int i = 0; i < 4; i++){
        for(int j = 0; j < 2; j++){
            pn->w_L2_L3[i][j] = g.gene_info[index];
            index++;
        }
    }
    index = 52;
    for(int i = 0; i < 4; i++){
        for(int j = 0; j < 2; j++){
            pn->b_L2_L3[i][j] = g.gene_info[index];
            index++;
        }
    }
